...
//Recoger datos de la declaracion de valor
...
import ArancelesService from "src/app/Impuestos/Aranceles/ArancelesServices";
//import Toast
...
//Import para hacer las peticiones de la declaración de valor
import Declaracion_ValorService from "../Declaracion_ValorService";
//Import de InputMask para las mascaras de los campos
...
function Declaracion_Valor_Editar() {
  ...
  //Trae todos los datos de la tabla pa editar
  ...
  const [ErrorLllenado, setErrorLllenado] = useState(false);

  const DiaglogErrorLllenado = () => {
    setErrorLllenado(!ErrorLllenado);
  }
  //Constante para guardar el id de la declarcion de valor insertada desde la tab 1
  const [Declaracion_Valor_Id, setDeclaracion_Valor_Id] = useState(data.deva_Id);
  ...
  const schemaValuesTab1 = yup.object().shape({
    ...
    DeclaracionMercancia: yup.string().required(),
  ...
  const FiltrarEstados = async (value) => {
    setProvinciaImportador_DDL(await load_DDLs.ProvinciasPorPais(value['value']))
  }
  ...
  const AduanaIngresoChange = (dato) => {
    setAduanaIngreso_DDL(Aduanas.filter((item) => item.value !== dato["value"]));
  };

  const AduanaSalidaChange = (dato) => {
    setAduanaSalida_DDL(Aduanas.filter((item) => item.value !== dato["value"]));
  };

  useEffect(() => {
    setAduanaIngreso_DDL(Aduanas);
    setAduanaSalida_DDL(Aduanas);
  }, [Aduanas])
  ...
  const schemaValuesTab2 = yup.object().shape({
    ...
    IdentificacionIntermedario: checkedIntermediario ? yup.string().required("").min(15, "Ingrese todo el DNI").max(15): yup.string().nullable() ,
    NombreIntermediario: checkedIntermediario ? yup.string().required() : yup.string().nullable(),
    DireccionIntermediario: checkedIntermediario ? yup.string().required() : yup.string().nullable(),
    PaisIntermediario: checkedIntermediario ? yup.object().required() : yup.object().nullable(),
    EstadoIntermediario: checkedIntermediario ? yup.object().required() : yup.object().nullable(),
    CorreoIntermediario: checkedIntermediario ? yup.string().required("").email("Ingrese un correo electronico valido") : yup.string().nullable() ,
    TelefonoIntermediario: checkedIntermediario ? yup.string().required("").min(14, "Ingrese un Numero de teléfono válido") : yup.string().nullable(),
    FaxIntermediario:checkedIntermediario ? yup.string().nullable() : yup.string().nullable(),
    TipoIntermediario: checkedIntermediario ? yup.object().required() : yup.object().nullable(),
    OtroTipoIntermediario:checkedIntermediario ? yup.string().nullable() : yup.string().nullable(),
  });
  ...
  const GuardarTab2 = async () => {
    if(checkedIntermediario){
      if(datosTab2.IdentificacionIntermedario == "" || datosTab2.CorreoIntermediario == "" || datosTab2.DireccionIntermediario == "" || datosTab2.NombreIntermediario == "" || datosTab2.EstadoIntermediario == null  || datosTab2.PaisIntermediario == null || datosTab2.TelefonoIntermediario == "" ){
      ToastWarning("Complete los campos vacios");
  ...
  const EditarTab2 = async () => {
    try {
      const response = await DeclaracionValorServices.EditarTab2DEVA(datosTab2, Declaracion_Valor_Id, checkedIntermediario);
      if (response.data.data.messageStatus == "1") {
          ...
          <Stack direction="row" spacing={1}>
            ...
            >
              ...
              <MenuItem onClick={() => handleEditarFacturas(params)}>
                <Icon>edit</Icon>ㅤEditar factura
              </MenuItem>
  ...
  const schemaValuesTab4_2 = yup.object().shape({
    ...
    UnidadMedida: yup.object().required().nullable(),
    ...
    EstadoMercaderia: yup.object().required().nullable(),
    OrigenMecaderia: yup.object().required().nullable(),
  ...
  const [numEditar, setnumEditar] = useState(0);
  ...
  //Constante UseForm para tabulacion 4_2
  const { handleSubmit: handleSubmitTab5, reset: resetTab5, control: controlTab5, formState: formStateTab5, watch: watchTab5, setValue: setValueTab5, trigger } = useForm({
  ...
  const [baseEstado, setbaseEstado] = useState(false);
  ...
  const [mensajeError,setmensajeErrorDialog] = useState("");
  ...
  const DialogSalir = async() => {
    ...
    if(isValidTab1){
      ...
      estado = true
      setmensaje1("• I. Aduana e Importador - Primer apartado")
    }
    if(isValidTab2){
      estado = false; 
    }
      ...
      estado = true
      setmensaje2("• I.I Proveedor e Intermediario - Segundo apartado")
    }
    if(isValidTab3){
      estado = false; 
    }
      ...
      estado = true
      setmensaje3("• I.II Característica de la transacción - Tercer apartado")
    }
    if(dataFacturas.length > 0){
      estado = false; 
    }
      ...
      estado = true
        if(!estadoFacturaItems){
          setmensaje4("• Factura (Las facturas no contiene items) - Cuarto apartado")
        }else{
          setmensaje4("• Factura (No hay facturas ingresadas)- Cuarto apartado")
        }    
    }
    if(isValidTab5){
      estado = false; 
      ...
      estado = true
      setmensaje5("• II. Condiciones de la transacción - Quinto apartado")
    }
    if(baseEstado){
      estado = false;
    }else{
  ...
  const Salir2 = async () => {
    ...
    if (isValidTab1 && isValidTab2 && isValidTab3 && dataFacturas.length > 0 && estadoFacturaItems && isValidTab5 && isValidTab6) {
      try {
      ...
      DiaglogErrorLllenado();
      ToastWarningPersonalizado("La declaracion no puede ser finalizada porque no ha terminado de digitar todo los datos")
    }
  }
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  //DDLS DEL PRIMER TAB------------------------------------------------------------------------------------
  const AduanasIngresoDDL = async () => {
    try {
      const response = await load_DDLs.aduanas()
      setAduanaIngreso_DDL(response)
      setValueTab1('AduanaIngreso', response.find(item => item.value === data.deva_AduanaIngresoId), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  const CargarArancel = async () => {
    try {
      const response = await ArancelSeraravices.listarDdl()
      setarancelesdata(response)
    } catch (error) {
      
    }
  };

  const AduanasSalidaDDL = async () => {
    try {
      const response = await load_DDLs.aduanas()
      setAduanaSalida_DDL(response)//tamalo
      setValueTab1('AduanaSalida', response.find(item => item.value === data.deva_AduanaDespachoId), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const NivelComercialImportadorDDL = async () => {
    try {
      const response = await load_DDLs.NivelesComerciales()
      setNivelComercialImportador_DDL(response)
      setValueTab1('NivelComercialImportador', response.find(item => item.value === data.nico_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  const PaisImportador = async () => {
    try {
      ...
      const XD =  response.find(item => item.value === data.impo_paisId);
  ...
  const ProvinciasImportador = async () => {
    try {

      const response = await load_DDLs.ProvinciasPorPais(data.impo_paisId)
      setProvinciaImportador_DDL(response)
      setValueTab1('EstadoImportador', response.find(item => item.value === data.impo_ciudId), { shouldValidate: true, shouldTouch: true })
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////
  //DDLS DEL Segundo TAB------------------------------------------------------------------------------------
  const CondicionComercialProveedorDDL = async () => {
    try {
      const response = await load_DDLs.CondicionesComerciales()
      setCondicionComercialProveedor_DDL(response)
      setValueTab2('CondicionComercialProveedor', response.find(item => item.value === data.coco_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const TipoIntermediario = async () => {
    try {
      const response = await load_DDLs.TipoIntermediario()
      setTipoIntermediario_DDL(response)
      setValueTab2('TipoIntermediario', response.find(item => item.value === data.tite_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const PaisProveedor = async () => {
    try {
      const response = await load_DDLs.paises()
      setPaisProveedor_DDL(response)
      const XD =  response.find(item => item.value === data.prov_paisId);
      setValueTab2('PaisProveedor',XD , { shouldValidate: true, shouldTouch: true })
      let xd = XD['label'].split('-')
  ...
  const ProvinciasProveedor = async () => {
    try {

      const response = await load_DDLs.ProvinciasPorPais(data.prov_paisId)
      setProvinciaProveedor_DDL(response)
      setValueTab2('EstadoProveedor', response.find(item => item.value === data.prov_ciudId), { shouldValidate: true, shouldTouch: true })
  ...
  const PaisIntermediario = async () => {
    try {
      const response = await load_DDLs.paises()
      setPaisIntermediario_DDL(response)
      const XD =  response.find(item => item.value === data.inte_paisId);
      setValueTab2('PaisIntermediario',XD , { shouldValidate: true, shouldTouch: true })
      let xd = XD['label'].split('-')
  ...
  const ProvinciasIntermediario = async () => {
    try {

      const response = await load_DDLs.ProvinciasPorPais(data.inte_paisId)
      setProvinciaIntermediario_DDL(response)
      setValueTab2('EstadoIntermediario', response.find(item => item.value === data.inte_ciudId), { shouldValidate: true, shouldTouch: true })
  ...
  const PaisEntrega = async () => {
    try {
      const response = await load_DDLs.paises()
      setPaisEntrega_DDl(response)
      setValueTab3('PaisEntrega', response.find(item => item.value === data.pais_EntregaId), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const Icoterms = async () => {
    try {
      const response = await load_DDLs.Incoterm()
      setIncoterm_DDL(response)
      setValueTab3('Incoterm', response.find(item => item.value === data.inco_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  const FormasEnvio = async () => {
    try {
      const response = await load_DDLs.FormasEnvio()
      setFormaEnvio_DDL(response)
      setValueTab3('FormaEnvio', response.find(item => item.value === data.foen_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  const FormasPago = async () => {
    try {
      const response = await load_DDLs.FormasPago()
      setFormasPago_DDL(response)
      setValueTab3('FormaPago', response.find(item => item.value === data.fopa_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  const PaisExportacion = async () => {
    try {
      const response = await load_DDLs.paises()
      setPaisExportacion_DDl(response)
      setValueTab3('PaisExportacion', response.find(item => item.value === data.pais_ExportacionId), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const MonedasTransaccion = async () => {
    try {
      const response = await load_DDLs.Monedas()
      setMonedasTransaccion_DDl(response)
      setValueTab3('MonedaTransaccion', response.find(item => item.value === data.mone_Id), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const CargarLugarEmbarque = async () => {
    try {
      const code = data.emba_Codigo.substring(0, 2)
      const response = await load_DDLs.ListarLugarEmbarque(code)
      setLugarEmbarque_DDl(response)
      setValueTab3("LugarEmbarque", response.find((Lugar) => Lugar.value === data.emba_Id), { shouldValidate: true, shouldTouch: true })
  ...
  useEffect(() => {
    ...
    setDeclaracion_Valor_Id(data.deva_Id)
    setcondicion_Id(data.codi_Id === null ? 0 : data.codi_Id)
    setbase_Id(data.base_Id === null ? 0 : data.base_Id)
    console.log(data);
    //CargarArancel()
    //Carga del primer tab---------------------------------------------------------------------------------------
    getAduanas();
    AduanasIngresoDDL();
    AduanasSalidaDDL();
    NivelComercialImportadorDDL();
    PaisImportador();
    ProvinciasImportador();
    setValueTab1("DeclaracionMercancia", data.deva_DeclaracionMercancia, { shouldValidate: true, shouldTouch: true });
    setValueTab1("FechaAceptacion", new Date(data.deva_FechaAceptacion), { shouldValidate: true, shouldTouch: true });
    setValueTab1("NombreImportador", data.impo_Nombre_Raso, { shouldValidate: true, shouldTouch: true });
    setValueTab1("RTNImportador", data.impo_RTN, { shouldValidate: true, shouldTouch: true });
    setValueTab1("NumeroRegistroImportador", data.impo_NumRegistro, { shouldValidate: true, shouldTouch: true });
    setValueTab1("DireccionImportador", data.impo_Direccion_Exacta, { shouldValidate: true, shouldTouch: true });
    setValueTab1("CorreoElectronicoImportador", data.impo_Correo_Electronico, { shouldValidate: true, shouldTouch: true });
    setValueTab1("TelefonoImportador", data.impo_Telefono, { shouldValidate: true, shouldTouch: true });
    setValueTab1("FaxImportador", data.impo_Fax, { shouldValidate: true, shouldTouch: true });
    setValueTab1("OtroNivelComercialImportado", data.impo_NivelComercial_Otro, { shouldValidate: true, shouldTouch: true });
    //-----------------------------------------------------------------------------------------------------------

    // Carga del segundo tab--------------------------------------------------------------------------------------

    if (data.prov_NumeroIdentificacion != null) {
      setValueTab2("IdentificacionProveedor", data.prov_NumeroIdentificacion, { shouldValidate: true, shouldTouch: true });
      setValueTab2("NombreProveedor", data.prov_Nombre_Raso, { shouldValidate: true, shouldTouch: true });
      setValueTab2("DireccionProveedor", data.prov_Nombre_Raso, { shouldValidate: true, shouldTouch: true });
      setValueTab2("CorreoProveedor", data.prov_Correo_Electronico, { shouldValidate: true, shouldTouch: true });
      setValueTab2("TelefonoProveedor", data.prov_Telefono, { shouldValidate: true, shouldTouch: true });
      setValueTab2("FaxProveedor", data.prov_Fax, { shouldValidate: true, shouldTouch: true });
      CondicionComercialProveedorDDL();
      PaisProveedor()
      ProvinciasProveedor()
    }
    ...
    if (data.inte_NumeroIdentificacion != null) {
      setcheckedIntermediario(true);
      setEliminarIntermediario(true)
      setValueTab2("IdentificacionIntermedario", data.inte_NumeroIdentificacion, { shouldValidate: true, shouldTouch: true });
      setValueTab2("NombreIntermediario", data.inte_Nombre_Raso, { shouldValidate: true, shouldTouch: true });
      setValueTab2("DireccionIntermediario", data.inte_Direccion_Exacta, { shouldValidate: true, shouldTouch: true });
      setValueTab2("NombreIntermediario", data.inte_Nombre_Raso, { shouldValidate: true, shouldTouch: true });
      setValueTab2("CorreoIntermediario", data.inte_Correo_Electronico, { shouldValidate: true, shouldTouch: true });
      setValueTab2("TelefonoIntermediario", data.inte_Telefono, { shouldValidate: true, shouldTouch: true });
      setValueTab2("FaxIntermediario", data.inte_Fax, { shouldValidate: true, shouldTouch: true });
      setValueTab2("OtroTipoIntermediario", data.inte_Tipo_Otro, { shouldValidate: true, shouldTouch: true });
      TipoIntermediario();
      PaisIntermediario()
      ProvinciasIntermediario()
    }


    // //-----------------------------------------------------------------------------------------------------------
    // //Carga del tercer tab---------------------------------------------------------------------------------------

    if (data.deva_LugarEntrega != null) {
      PaisEntrega();
      Icoterms();
      FormasEnvio();
      FormasPago();
      ...
      CargarLugarEmbarque();
      MonedasTransaccion();
      setValueTab3("LugarEntrega", data.deva_LugarEntrega, { shouldValidate: true, shouldTouch: true });
      setValueTab3("Version", data.inco_Version, { shouldValidate: true, shouldTouch: true });
      setValueTab3("NumeroContrato", data.deva_NumeroContrato, { shouldValidate: true, shouldTouch: true });
      setValueTab3("FechaContrato", new Date(data.deva_FechaContrato), { shouldValidate: true, shouldTouch: true });
      setValueTab3("OtraFormaEnvio", data.deva_FormaEnvioOtra, { shouldValidate: true, shouldTouch: true });
      setValueTab3("OtraFormaPago", data.deva_FormaPagoOtra, { shouldValidate: true, shouldTouch: true });
      setChecked(data.deva_PagoEfectuado, { shouldValidate: true, shouldTouch: true })
      setValueTab3("FechaExportacion", new Date(data.deva_FechaExportacion), { shouldValidate: true, shouldTouch: true });
      setValueTab3("OtraMoneda", data.mone_Otra, { shouldValidate: true, shouldTouch: true });
      setValueTab3("CambioMoneda", data.deva_ConversionDolares, { shouldValidate: true, shouldTouch: true });
    }
    // //-----------------------------------------------------------------------------------------------------------
    // //Carga del cuarto tab---------------------------------------------------------------------------------------
    ...
    // //Carga del Quito tab---------------------------------------------------------------------------------------
    if (data.codi_Id != null) {
      setcampo24(data.codi_Restricciones_Utilizacion === true ? true : false, { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo24", data.codi_Restricciones_Utilizacion ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo24_1", data.codi_Indicar_Restricciones_Utilizacion, { shouldValidate: true, shouldTouch: true })
      //----
      setcampo25(data.codi_Depende_Precio_Condicion ? true : false, { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo25", data.codi_Depende_Precio_Condicion ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo25_1", data.codi_Indicar_Existe_Condicion, { shouldValidate: true, shouldTouch: true })
      //----

      setValueTab5("campo26", data.codi_Condicionada_Revertir ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      //----
      setcampo27(data.codi_Vinculacion_Comprador_Vendedor ? true : false, { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo27", data.codi_Vinculacion_Comprador_Vendedor ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      if (data.codi_Vinculacion_Comprador_Vendedor == true)
        setValueTab5("campo27_2", data.codi_Vinculacion_Influye_Precio ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      //----
      setcampo28(data.codi_Pagos_Descuentos_Indirectos ? true : false, { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo28", data.codi_Pagos_Descuentos_Indirectos ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo28_1", data.codi_Concepto_Monto_Declarado, { shouldValidate: true, shouldTouch: true })
      //----
      setcampo29(data.codi_Existen_Canones ? true : false, { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo29", data.codi_Existen_Canones ? "True" : "False", { shouldValidate: true, shouldTouch: true });
      setValueTab5("campo29_1", data.codi_Indicar_Canones, { shouldValidate: true, shouldTouch: true })
    }
    ...
    if (data.base_Id != null) {
      setbaseEstado(true);
      setValueTab6("campo39", data.base_PrecioFactura, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo40", data.base_PagosIndirectos, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_1", data.base_MontCondicion, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_2", data.base_MontoReversion, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_3", data.base_ComisionCorrelaje, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_4", data.base_Gasto_Envase_Embalaje, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_5", data.base_ValoresMateriales_Incorporado, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_6", data.base_Valor_Materiales_Utilizados, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_7", data.base_Valor_Materiales_Consumidos, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_8", data.base_Valor_Ingenieria_Importado, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_9", data.base_Valor_Canones, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_10", data.base_Gasto_TransporteM_Importada, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_11", data.base_Gastos_Carga_Importada, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo42_12", data.base_Costos_Seguro, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo44_1", data.base_Gastos_Asistencia_Tecnica, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo44_2", data.base_Gastos_Transporte_Posterior, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo44_3", data.base_Derechos_Impuestos, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo44_4", data.base_Monto_Intereses, { shouldValidate: true, shouldTouch: true })
      setValueTab6("campo44_5", data.base_Deducciones_Legales, { shouldValidate: true, shouldTouch: true })
    } else {
      valoresTab6()
    }
    ...
    <Card sx={{ minWidth: 275, margin: '40px' }}>
            ...
            <form onSubmit={handleSubmitTab1((_data) => { })}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={3}>
                  ...
                  <FormControl fullWidth>
                    <FormLabel id="group-label" error={!!errorsTab1.DeclaracionMercancia}>Declaracion de mercancia</FormLabel>
                    <Controller
                      render={({ field }) => (
                        <TextField
                          {...field}
                      ...
                      render={({ field }) => (
                        <Autocomplete
                          ...
                          onChange={(event, value) => {
                            ...
                            setValueTab2("EstadoIntermediario", null)
                ...
                <Grid item xs={4}>
                  ...
                  <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'end', alignItems: 'center' }}>
                    ...
                    {isEdit ? <Button
                      ...
                      onClick={() => {
                        setisEdit(false);
                        console.log(numEditar); 
                        resetTab4_2(DefaultValuesTab4_2);
                        setValueTab4_2("NumeroItem",numEditar + 1,{ shouldValidate: true, shouldTouch: true })
                        setcheckedAuto(false);
                        setcantidadDisabed(true);
                        setcheckedCambbioAranceles(true);
                        setcheckedCigarros(false);
                  ...
                  <Controller
                    render={({ field }) => (
                      <FormControl error={!!errorsTab5.campo27_2} fullWidth={true}>
                        <FormLabel></FormLabel>
                        <div className={`${classes.radioContainer} ${!!errorsTab5.campo27_1 ? classes.radioError : ''}`}>
                          <RadioGroup
                            row
                            name="simple-radio"
                            aria-label="simple-radio"
                ...
                <div className="flex flex-col items-center">
                  ...
                  >
                    <div className="mt-4 text-4xl sm:text-7xl font-extrabold tracking-tight leading-tight text-center">
                      ¡Muchas gracias por editar tu declaracion de valor!
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      ...
                      <br />
                      <br />
                      ¿Deseas salir de la edición de tu declaración de valor?
                      <br />
                    </Typography>
                  </motion.div>
      ...
      >
        ...
        <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }}
                  >
            <div className="mt-3 text-1xl sm:text-5xl font-extrabold tracking-tight leading-tight text-center">
                    Declaracion de valor incompleta
              </div>
          </motion.div> 
        </DialogTitle>
        <DialogContent>
        <DialogContentText id="alert-dialog-description">
          <motion.div
            ...
            animate={{ opacity: 1, transition: { delay: 0.15 } }}
          >
            <Typography className="mt-6 sm:text-2xl tracking-tight" color="text.secondary">
              Para finalizar su declaración de valor debe terminar de ingresar los siguientes formularios:
              <br></br>
              {mensaje1 && <>
                <br></br>
                {mensaje1}
...
export default Declaracion_Valor_Editar;
