...
import NivelesComercialesService from "./NivelesComercialesService";
import { FileTextFilled } from "@ant-design/icons";
import { MoreVert } from "@material-ui/icons";
import { ExportToCsv } from 'export-to-csv';
import ExportToExcel from "./NivelesComercialesExcel";
import PDFGenerator from "./NivelesComercialesPDF";

/* Campos del formulario*/
const defaultNivelesComercialesValues = {
...
const accountSchema = yup.object().shape({
  id: yup.string(),
  nico_Codigo: yup.string().required(""),
  nico_Descripcion: yup.string().trim().required(""),
});

function NivelesComercialesIndex() {
  const nivelesComercialesServices = NivelesComercialesService();
  ...
  const VisibilidadTabla = () => {
    ...
    reset(defaultNivelesComercialesValues);
  };
  ...
  const handleEdit = (datos) => {
    setEditar(true);
    VisibilidadTabla();
    setValue("id", datos["nico_Id"]);
    setValue('nico_Codigo', datos["nico_Codigo"]);
    setValue('nico_Descripcion', datos["nico_Descripcion"]); // Corregir aquí
    handleClose(datos.nico_Id);
  };
  ...
  const columns = [
    ...
    {
      title: "Código del nivel comercial",
      dataIndex: "nico_Codigo",
      key: "nico_Codigo",
      sorter: (a, b) => a.nico_Codigo.localeCompare(b.nico_Codigo), //sorting para Letras
    },
    {
      title: "Descripción del nivel comercial",
      dataIndex: "nico_Descripcion",
      key: "nico_Descripcion",
      sorter: (a, b) => a.nico_Descripcion.localeCompare(b.nico_Descripcion),//sorting para Letras
    },
    {
      title: "Acciones",
      key: "operation",
      render: (params) => (
        <div key={params.nico_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              aria-controls={`menu-${params.nico_Id}`}
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.nico_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.nico_Id}`}
              anchorEl={anchorEl[params.nico_Id]}
              keepMounted
              open={Boolean(anchorEl[params.nico_Id])}
              onClose={() => handleClose(params.nico_Id)}
            >
  ...
  const csvHeader = [
    ...
    {
      label: 'Código del nivel comercial',
    },
    {
      label: 'Descripción del nivel comercial'
    }
  ]

  const csvOptions = {
    filename: 'Niveles_Comerciales',
    fieldSeparator: ',',
    quoteStrings: '"',
    decimalSeparator: '.',
    showLabels: true,
  ...
  const camposToFilter = ["key", "nico_Codigo", "nico_Descripcion"];
  ...
  const { handleSubmit, register, reset, control, watch, formState, setValue } =
    useForm({
      defaultNivelesComercialesValues, //Campos del formulario
      mode: "all",
  ...
  const NivelesComercialesGetData = async () => {
    try {
      setCargandoData([]);
      setData([]);

      const data = await nivelesComercialesServices.listar();

      setData(data);
      data.length > 0 ? setCargandoData(data) : setCargandoData(null);
      SetExportData(await nivelesComercialesServices.ExportData());
    } catch (error) {
  ...
  const NivelesComercialesCreate = async () => {
    try {
      const response = await nivelesComercialesServices.crear(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccessGuardado();
        NivelesComercialesGetData();
        VisibilidadTabla();
        reset(defaultNivelesComercialesValues);
  ...
  const NivelesComercialesEdit = async () => {
    try {
      const response = await nivelesComercialesServices.editar(datosWatch);
      if (response.data.data.messageStatus == 1) {
        ToastSuccessEditar();
        NivelesComercialesGetData();
        VisibilidadTabla();
  ...
  const NivelesComercialesDelete = async () => {
    try {
      const response = await nivelesComercialesServices.eliminar(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccessEliminar();
        NivelesComercialesGetData();
        DialogEliminar();
  ...
  const GuardarNivelesComerciales = () => {
    if (isValid) {
      // Validacion de campos completos
      if (!editar) {
        // Validacion de la funcion a realizar
        NivelesComercialesCreate();
      } else {
        NivelesComercialesEdit();
      }
    } else {
  ...
  return (
    <Card sx={{ minWidth: 275, margin: "40px" }}>
      <CardMedia
        component="img"
        height="200"
        image="https://i.ibb.co/4Ky65F5/NIVELES-COMERCIALES.png"
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={
                      editar ? "Editar Niveles Comerciales" : "Agregar Niveles Comerciales"
                    }
                  />
                </Divider>
              </Grid>
              <Grid item xs={6}>
                ...
                <FormControl fullWidth>
                  <FormLabel error={!!errors.nico_Codigo}>
                    Codigo Comercial:
                  </FormLabel>
                  <Controller
                    render={({ field }) => (
              ...
              <Grid item xs={6}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.nico_Descripcion}>
                    Nombre del Nivel Comercial:
                  </FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
              ...
              >
                <Button
                  ...
                  onClick={GuardarNivelesComerciales}
                >
                  Guardar
                </Button>
        ...
        <DialogActions>
          ...
          >
            <Button
              ...
              color="error"
              style={{ borderRadius: "10px", marginRight: "10px" }}
              onClick={NivelesComercialesDelete}
            >
              Eliminar
            </Button>
              ...
              <Box sx={{ textAlign: "center" }}>
                ...
                <InputLabel htmlFor="id">
                  <Typography sx={{ fontWeight: "bold", color: "#000000" }}>
                    Id del Nivel Comercial:
                  </Typography>
                  <Typography>{DatosDetalles["nico_Id"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
            <Grid
            ...
            >
              <Box sx={{ textAlign: "center" }}>
                ...
                <InputLabel htmlFor="descripcion">
                  ...
                  <Typography>{DatosDetalles["nico_Codigo"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
            <Grid
            ...
            >
              <Box sx={{ textAlign: "center" }}>
                <InputLabel htmlFor="descripcion">
                  ...
                  <Typography>{DatosDetalles["nico_Descripcion"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
              ...
              >
                ...
                <tbody>
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["nico_FechaCreacion"]
                        ? new Date(
                          DatosDetalles["nico_FechaCreacion"]
                        ).toLocaleString()
                        : ""}
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["nico_FechaModificacion"]
                        ? new Date(
                          DatosDetalles["nico_FechaModificacion"]
                        ).toLocaleString()
                        : ""}
