...
function FacturaExportacionEditar() {
    ...
    useEffect(() => {
        OrdenesCompraDDL();
        setValue('faex_Id', myData.faex_Id);
        setValue('duca_No_Duca', myData.duca_No_Duca == 'N/A' ? '' : myData.duca_No_Duca);
        setValue('faex_Fecha', myData.faex_Fecha);
        setValue('orco_Id', {value: myData.orco_Id, label: myData.orco_Descripcion});

        PODetallesDDL(myData.faex_Id)
        DetallesGetData(myData.faex_Id)
        setValue1('faex_Id', myData.faex_Id)
        Setprev_orco_Id({value: myData.orco_Id, label: myData.orco_Descripcion})
    ...
    const EditarHeaderFactura = async () =>{
        try{
            ...
            if (response.data.data.messageStatus == "1") {
                ToastSuccessEditar();
                PODetallesDDL(datosWatch["faex_Id"]);
                CleanTab2();
                setValor(1);
                setCambiar(false);
                Setprev_orco_Id(datosWatch["orco_Id"]);
...
export default FacturaExportacionEditar