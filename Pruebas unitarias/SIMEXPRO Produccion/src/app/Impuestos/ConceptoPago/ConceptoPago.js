...
import ConceptoPagoService from "./ConceptoPagoService";

/* Campos del formulario*/
const defaultConceptoPagoValues = {
...
const accountSchema = yup.object().shape({
  id: yup.string(),
  copa_Descripcion: yup.string().trim().required(""),
});

function ConceptoPagoIndex() {
  const conceptoPagoService = ConceptoPagoService();
  ...
  const VisibilidadTabla = () => {
    ...
    reset(defaultConceptoPagoValues);
  };
  ...
  const handleEdit = (datos) => {
    ...
    setValue("id", datos["copa_Id"]);
    setValue("copa_Descripcion", datos["copa_Descripcion"]);
    handleClose(datos.copa_Id);
  };
  ...
  const columns = [
    ...
    {
      title: "Descripción del concepto de pago",
      dataIndex: "copa_Descripcion",
      key: "copa_Descripcion",
      sorter: (a, b) => a.copa_Descripcion.localeCompare(b.copa_Descripcion), //sorting para Letras
    },
    {
      title: "Acciones",
      key: "operation",
      render: (params) => (
        <div key={params.copa_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              aria-controls={`menu-${params.copa_Id}`}
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.copa_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.copa_Id}`}
              anchorEl={anchorEl[params.copa_Id]}
              keepMounted
              open={Boolean(anchorEl[params.copa_Id])}
              onClose={() => handleClose(params.copa_Id)}
            >
  ...
  const csvHeader = [
    ...
    {
      label: 'Descripción del concepto de pago'
    }
  ]
  const csvOptions = {
    filename: 'Concepto_Pago',
    fieldSeparator: ',',
    quoteStrings: '"',
    decimalSeparator: '.',
    showLabels: true,
  ...
  const camposToFilter = ["key", "copa_Descripcion"];
  ...
  const { handleSubmit, register, reset, control, watch, formState, setValue } =
    useForm({
      defaultConceptoPagoValues, //Campos del formulario
      mode: "all",
  ...
  const ConceptoPagoGetData = async () => {
    try {
      setCargandoData([]);
      setData([]);

      const data = await conceptoPagoService.listar();

      setData(data);
      data.length > 0 ? setCargandoData(data) : setCargandoData(null);
      SetExportData(await conceptoPagoService.ExportData());
    } catch (error) {
  ...
  const ConceptoPagoCreate = async () => {
    try {
      const response = await conceptoPagoService.crear(datosWatch);
  ...
  const conceptoPagoEdit = async () => {
    try {
      const response = await conceptoPagoService.editar(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccessEditar();
        ConceptoPagoGetData();
        VisibilidadTabla();
  ...
  return (
    <Card sx={{ minWidth: 275, margin: "40px" }}>
      <CardMedia
        component="img"
        height="200"
        image="https://i.ibb.co/CtrbQ3k/CONCEPTO-DE-PAGO.png"
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={editar ? "Editar Concepto de Pago" : "Agregar Concepto de pago"}
                  />
                </Divider>
              </Grid>
              <Grid item xs={2}>
              ...
              <Grid item xs={8}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.copa_Descripcion}>Descripción:</FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
                        {...field}
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} style={{ marginBottom: "30px" }}>
              <Divider style={{ marginTop: "0px", marginBottom: "10px" }}>
                <Chip label="Detalles de el concepto de pago" />
              </Divider>
            </Grid>
              ...
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="id">
                  <Typography sx={{ fontWeight: "bold", color: "#000000" }}>
                    Id de el concepto de pago:
                  </Typography>
                  <Typography>{DatosDetalles["copa_Id"]}</Typography>
                </InputLabel>
              </Box>
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="descripcion">
                  <Typography sx={{ fontWeight: "bold", color: "#000000" }}>
                    Nombre de el concepto de pago:
                  </Typography>
                  <Typography>{DatosDetalles["copa_Descripcion"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
              ...
              >
                ...
                <tbody>
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["usuaCreacion"]}
                    </td>
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["copa_FechaCreacion"]
                        ? new Date(
                          DatosDetalles["copa_FechaCreacion"]
                        ).toLocaleString()
                        : ""}
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["usuaModificacion"]}
                    </td>
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["copa_FechaModificacion"]
                        ? new Date(
                          DatosDetalles["copa_FechaModificacion"]
                        ).toLocaleString()
                        : ""}
