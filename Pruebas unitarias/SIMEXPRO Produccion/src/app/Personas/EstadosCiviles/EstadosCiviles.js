...
//Imports de los exports (pdf, cvs, excel)
...
// import del archivo para los services
import EstadosCivilesService from "./EstadosCivilesService";

/* Campos del formulario*/
const defaultEstadosCivilesValues = {
...
const accountSchema = yup.object().shape({
  id: yup.string(),
  escv_Nombre: yup.string().trim().required(""),
});

function EstadosCivilesIndex() {

  const estadosCivilesService = EstadosCivilesService();
  ...
  const VisibilidadTabla = () => {
    ...
    reset(defaultEstadosCivilesValues);
  };
  ...
  const handleEdit = (datos) => {
    ...
    setValue("id", datos["escv_Id"]);
    setValue("escv_Nombre", datos["escv_Nombre"]);
        ...
        <div key={params.escv_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              ...
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.escv_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.escv_Id}`}
              anchorEl={anchorEl[params.escv_Id]}
              keepMounted
              open={Boolean(anchorEl[params.escv_Id])}
              onClose={() => handleClose(params.escv_Id)}
            >
  ...
  const csvHeader = [
    ...
    {
      label: 'Nombre del estado civíl'
    }
  ]
  const csvOptions = {
    filename: 'Estados_Civiles',
    fieldSeparator: ',',
    quoteStrings: '"',
    decimalSeparator: '.',
    showLabels: true,
  ...
  const { handleSubmit, reset, control, watch, formState, setValue } =
    useForm({
      defaultEstadosCivilesValues, //Campos del formulario
      mode: "all",
  ...
  const EstadosCivilesGetData = async () => {
    try {
      setCargandoData([]);
      setData([]);
      const data = await estadosCivilesService.listar()
      setData(data);
      data.length > 0 ? setCargandoData(data) : setCargandoData(null);
      SetExportData(await estadosCivilesService.ExportData());
    } catch (error) {
  ...
  const EstadosCivilesCreate = async () => {
    try {
      const response = await estadosCivilesService.crear(datosWatch);
  ...
  const EstadosCivilesEdit = async () => {
    try {
      const response = await estadosCivilesService.editar(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccessEditar();
        EstadosCivilesGetData();
        VisibilidadTabla();
  ...
  const estadosCivilesDelete = async () => {
    try {
      const response = await estadosCivilesService.eliminar(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccessEliminar();
        EstadosCivilesGetData();
        DialogEliminar();
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={editar ? "Editar estado civil" : "Agregar estado civil"}
                  />
                </Divider>
              </Grid>
              <Grid item xs={2}>
              ...
              <Grid item xs={8}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.escv_Nombre}>Nombre del estado civil:</FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
                        {...field}
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} style={{ marginBottom: "30px" }}>
              <Divider style={{ marginTop: "0px", marginBottom: "10px" }}>
                <Chip label="Detalles de el Estado Civil" />
              </Divider>
            </Grid>
              ...
              >
                ...
                <tbody>
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["escv_FechaModificacion"]
                        ? new Date(
                          DatosDetalles["escv_FechaModificacion"]
                        ).toLocaleString()
                        : ""}
