...
function OrdenCompra_Editar() {
  ...
  const [CodigoBloqueado, setCodigoBloqueado] = useState(true);
  ...
  /* Constante para el swich de si brindara materiales*/ //-------  Cambiar
  ...
  const [OrdenCompraID, setOrdenCompraID] = useState(data.orco_Id);
  ...
  const schemaOrdenCompraEncabezado = yup.object().shape({
    ...
    FechaComenzar: yup.date().required(),
    //FechaLimite: yup.date(),

    FechaLimite: yup.date().required("").test('fecha-limite-mayor', 'La fecha límite debe ser mayor que la fecha de comienzo', function (value) {
  ...
  const ClientesDDL = async () => {
    try {
      const response = await load_DDLs.Clientes()
      setClientes_DDL(response)
      setValueEncabezado('Cliente', response.find(item => item.value === data.orco_IdCliente), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const MetodoPagoDDL = async () => {
    try {
      const response = await load_DDLs.FormasPago()
      setMetodoPago_DDL(response)
      setValueEncabezado('TipoPago', response.find(item => item.value === data.orco_MetodoPago), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
      
    }
  };

  const EmbalajeDDL = async () => {
    try {
      const response = await load_DDLs.TipoEmbalaje()
      setEmbajale_DDL(response)
      setValueEncabezado('Embalaje', response.find(item => item.value === data.orco_IdEmbalaje), { shouldValidate: true, shouldTouch: true })
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setValueEncabezado("Codigo", data.orco_Codigo, { shouldValidate: true, shouldTouch: true })
    setValueEncabezado("FechaComenzar", new Date(data.orco_FechaEmision), { shouldValidate: true, shouldTouch: true })
    setValueEncabezado("FechaLimite", new Date(data.orco_FechaLimite), { shouldValidate: true, shouldTouch: true })
    setValueEncabezado("Direccion", data.orco_DireccionEntrega, { shouldValidate: true, shouldTouch: true })

    ListadoOrdenesCompraDetalles(data.orco_Id);

    if (data.orco_Materiales) {
      setChecked(true)
      SetHabilitarMate(false)
    }
  ...
  const { handleSubmit: handleSubmitDetalles, register: registerDetalles, reset: resetDetalles, control: controlDetalles, watch: watchDetalles, formState: formStateDetalles, setValue: setValueDetalles } = useForm({
    OrdenCompraDetalle, //Campos del formulario
    mode: "all",
  ...
  //Constante cuando se hace click para el boton de opciones detalles
  const handleClick = (event, id) => {
   ...
   //Cerrar espacio de opciones de detalles
  ...
  const camposToFilterDocumentos = ["dopo_Id"];
...
export default OrdenCompra_Editar;
