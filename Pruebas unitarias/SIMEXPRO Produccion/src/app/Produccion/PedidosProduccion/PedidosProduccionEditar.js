...
function PedidosProduccionEditar() {
    const load_DDLs = Load_DDLs()

    const theme = useTheme();
    const [Valor, setValor] = React.useState(0);

    //se utiliza para recibir los datos para editarlos 
    const location = useLocation();

    //aqui se almacena el json con el registroa a editar
    ...
    //variable usada para bloquear el primer tap cuanso este valido
    const [Tap1Activado, setTap1Activado] = useState(false);
    ...
    useEffect(() => {
        LotesGet()
        EmpleadosGet()
        setValue('ppro_Fecha', myData.ppro_Fecha);
        setValue('ppro_Id', myData.ppro_Id);
        setValue('ppro_Estados', myData.ppro_Estados);
        setValue('ppro_Observaciones', myData.ppro_Observaciones);

        setValue('Empleado', { value: parseInt(myData.empl_Id), label: myData.empl_NombreCompleto })

        DetallesGetData(myData.ppro_Id)
        setValue1('ppro_Id', myData.ppro_Id)
    }, []);
        ...
        try {
            if (Id !== undefined && Id !== "" && Id !== null) { // Corrección en la condición
                ...
                if (dd.length <= 0) {
                    ...
                    //setValue1('Material', { value: dd[0].lote_CodigoLote, label: dd[0].mate_Descripcion });
    ...
    //Peticion para crear el encabezado de pedidos produccionif (LoteId == datosWatch1['lote_Id']) {
...
export default PedidosProduccionEditar