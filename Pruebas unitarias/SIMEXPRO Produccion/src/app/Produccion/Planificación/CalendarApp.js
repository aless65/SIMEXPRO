...
function CalendarApp(props) {
  ...
  const handleEventDrop = (eventDropInfo) => {
    // eventDropInfo.event.extendedProps.fechalimite =  eventDropInfo.event.end;
    const { id, title, label, allDay, start, end, extendedProps, } = eventDropInfo.event;
    // extendedProps.fechalimite: end;
    dispatch(
      updateEvent({
        id,
        title,
  ...
  function renderEventContent(eventInfo) {
    ...
    return (
      ...
      >
        ...
        <Typography>
          <BootstrapTooltip title={`${eventInfo.event.title} ${eventInfo.event.extendedProps.descripcionDetalle}`}>
            <span style={{ marginLeft: 'auto' }}>
              <InfoIcon sx={{ fontSize: '11px' }} />
            </span>
          </BootstrapTooltip>
        </Typography>
        {/* <Typography className="text-12 px-4 truncate"><InfoIcon/></Typography> */}
      </Box>
    );
  }
  ...
  const handleEventChange = (changeInfo) => {
    const { id, title, label, allDay, start, end, extendedProps, } = changeInfo.event;
    dispatch(
      updateEvent({
        id,
        title,
