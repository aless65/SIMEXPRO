...
function Reporte_CostosMaterialesNoBrindados() {
  const [data, setData] = useState([]);
  const [ddl, setDdl] = useState([]);
  const user = useSelector(selectUser);
  const [mate_FechaInicio, setmate_FechaInicio] = useState("");
  const [mate_FechaLimite, setmate_FechaLimite] = useState("");
  ...
  const getdata = async (fechainicio, fechafin) => {
    try {
      ...
      const data = await reportesProduccionService.CostosMaterialesNoBrindados(
        fechainicio,
        fechafin
      );
      setData(data);
  ...
  const MyDoc = () => (
    <Document
      title="ReporteCostosdeMateriales.pdf"
      creator="SIMEXPRO"
      author="SIMEXPRO"
    >
      <Page size="A4" style={{ ...styles.page }}>
        ...
        <View style={styles.columnsContainer3} fixed>
          <Text style={styles.detallesTitle}>Reporte Costos de Materiales</Text>
          <Text style={styles.headerTextoDerecho}>
            DATOS BASADOS EN EL RANGO DE:
          </Text>
          <Text style={styles.headerTextoDerecho}>
            {mate_FechaInicio} - {mate_FechaLimite}
          </Text>
        </View>
        ...
        <View style={{ display: "flex" }}>
          ...
          >
            ...
            >
              ...
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Cantidad Pedida</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Valor Promedio</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Porcentaje de Pedido</Text>
              </View>
            </View>
            ...
            {data && data.length > 0 ? (
              data.map((datos, Index) => (
                ...
                >
                  ...
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>
                    {datos.precioPromedioMaterial} L.
                    </Text>
                  </View>
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>
                    {datos.porcentajeProductos}%
                    </Text>
                  </View>
                </View>
              ))
    ...
    <>
      <Card sx={{ minWidth: 275, margin: "40px" }}>
        <CardMedia
          ...
          height="200"
          className="mb-24"
          image="https://i.ibb.co/fCmFqdx/COSTOS-DE-PRODUCCI-N.png"
            ...
            <RangePicker
              ...
              onChange={(value) => {
                ...
                if (value && value.length === 2) {
                  ...
                  setmate_FechaInicio(value[0].format("YYYY-MM-DD"));
                  setmate_FechaLimite(value[1].format("YYYY-MM-DD"));
                  setPDF(true);
                } else {
                  setmate_FechaInicio("");
                  setmate_FechaLimite("");
...
export default Reporte_CostosMaterialesNoBrindados;
