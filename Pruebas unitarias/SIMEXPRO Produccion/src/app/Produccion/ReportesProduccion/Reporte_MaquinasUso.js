...
function MaquinasUso() {
  ...
  const getdata = async (param) => {
    ...
    try {
      ...
      const data = await reportesProduccionService.MaquinasUso(param);
      setData(data);
      ...
      Total = EnUso + NoEnUso;
      EnUso = (EnUso / Total) * 100;
      NoEnUso = (NoEnUso / Total) * 100;
      const labels = ['En Uso', 'No en Uso'];
      const datasets = [EnUso, NoEnUso];
      ...
      const chartconfig = {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Porcentaje de Maquinas Segun su Estado',
              data: datasets,
              backgroundColor: ['#588CBC80', '#FED9B780'],
        ...
        options: {
          title: {
            ...
            display: true,
            position: 'top',
            text: 'Porcentaje de Maquinas Segun su Estado',
          },
          plugins: {
            legend: false,
            outlabels: {
  ...
  const ddlget = async () => {
    try {
      ...
      const data = await load_DDLs.Modulos();
      setDdl(data);
    } catch (error) {
  ...
  const MyDoc = () => (
    <Document title="ReporteTiemposMaquina.pdf" creator="SIMEXPRO" author="SIMEXPRO">
      <Page size="A4" style={{ ...styles.page }}>
        ...
        <View style={styles.columnsContainer3} fixed>
          <Text style={styles.detallesTitle}>Reporte Estado Maquinas</Text>
        ...
        <View style={{ display: 'flex' }}>
          ...
          >
            <Text style={{ ...styles.tableTitle, fontWeight: 'bold' }}>
              {selectedId === 0
                ? "Registro de todas las Maquinas"
                : data[0] &&
                `Registro de Maquinas ${data[0]?.modu_Nombre}`}
          ...
          <View style={{ ...styles.tableContainerTransparente, textTransform: 'uppercase' }}>
            ...
            >
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>No. de Serie</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Marca</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>En Uso</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Deshabilitada</Text>
              </View>
              <View style={styles.tableCellRegistors}>
                <Text style={styles.cellText}>Habilitada</Text>
              </View>
            </View>
            ...
            {data && data.length > 0 ? (
              data.map((datos, Index) => (
                ...
                >
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>{datos.maqu_NumeroSerie}</Text>
                  </View>
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>{datos.marq_Nombre}</Text>
                  </View>
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>
                      {datos.enUso === false ? 'NO' : datos.enUso === true ? 'SI' : ''}
                    </Text>
                  </View>
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>
                      {data.length > 0 && data[Index].deshabilitada
                        ? data[Index].deshabilitada.slice(0, data[Index].deshabilitada.indexOf('T'))
                        : ' '}
                    </Text>
                  </View>
                  <View style={styles.tableCellRegistors}>
                    <Text style={styles.cellText}>
                      {
                        datos.enUso === false
                          ? 'INDEFINIDO'
                          : datos.enUso === true && data.length > 0 && data[Index].habilitada
                            ? data[Index].habilitada.slice(0, data[Index].habilitada.indexOf('T'))
                            : null // Puedes manejar otros casos si es necesario, como dejarlo en blanco ('') o mostrar un mensaje de error.
                      }
                    </Text>
                  </View>
                </View>
        ...
        <Grid container spacing={3}>
          <Grid item xs={6} md={6} className="mx-auto">
            <FormControl fullWidth>
              <FormLabel className="mt-5">Modulo</FormLabel>
              <Select
                size="small"
                value={selectedId}
                onChange={(event) => {
