...
function RevisionCalidadCrear() {
  ...
  const defaultValues = {
    id: urlData.reca_Id,
    image: urlData.reca_Imagen,
    cantidad: urlData.reca_Cantidad,
    fechaRevision: urlData.reca_FechaRevision,
    observaciones: urlData.reca_Descripcion,
    codigoproceso: urlData.ensa_Id,
    scrap: urlData.reca_Scrap,
  };
  ...
  const [image, setimage] = useState(urlData.reca_Imagen);
  ...
  async function RevisionEdit() {
    try {
      const response = await revisionCalidadService.editar(modelo);
      if (response.data?.data?.messageStatus === "1") {
        ToastSuccessEditar();
        History.push("RevisionCalidad");
      } else if (response == "No se pudo subir la imagen") {
        ToastErrorPersonalizado("Hubo un error con el servidor de imagenes.");
      } else {
        ToastError("Ocurrio un error respuesta");
      }
    } catch (error) {
                        ...
                        render={({ field }) => (
                          <>
                            ...
                            >
                              ...
                              <Switch {...field} checked={modelo.scrap} />
