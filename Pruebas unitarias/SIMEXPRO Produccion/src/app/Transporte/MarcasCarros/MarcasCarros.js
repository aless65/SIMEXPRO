...
//Import service
import MarcasCarrosServices from "./MarcasCarrosService";
//Import ddls
...
import ExportToExcel from "./MarcasCarrosExcel";
import PDFGenerator from "./MarcasCarrosPDF";

const defaultMarcasCarrosValues = {
  id: "",
  marc_Descripcion: "",
}
...
const accountSchema = yup.object().shape({
  id: yup.string(),
  marc_Descripcion: yup.string().trim().required(""),
});
...
function MarcasCarrosIndex() {
  ...
  const marcasCarrosServices = MarcasCarrosServices()
  ...
  const VisibilidadTabla = () => {
    ...
    reset(defaultMarcasCarrosValues);
  };
  ...
  const handleEdit = (datos) => {
    ...
    setValue("id", datos["marc_Id"]);
    setValue("marc_Descripcion", datos["marc_Descripcion"]);
    handleClose(datos.marc_Id);
  };
  ...
  const columns = [
    ...
    {
      title: "Nombre de la marca de carro",
      dataIndex: "marc_Descripcion",
      key: "marc_Descripcion",
      sorter: (a, b) => a.marc_Descripcion.localeCompare(b.marc_Descripcion), //sorting para Letras
    },
    {
      title: "Acciones",
      key: "operation",
      render: (params) => (
        <div key={params.marc_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              aria-controls={`menu-${params.marc_Id}`}
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.marc_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.marc_Id}`}
              anchorEl={anchorEl[params.marc_Id]}
              keepMounted
              open={Boolean(anchorEl[params.marc_Id])}
              onClose={() => handleClose(params.marc_Id)}
            >
  ...
  const csvHeader = [
    ...
    {
      label: 'Nombre de la marca de carro'
    }
  ]

  const csvOptions = {
    filename: 'Marcas_Carros',
    fieldSeparator: ',',
    quoteStrings: '"',
    decimalSeparator: '.',
    showLabels: true,
  ...
  const camposToFilter = ["key", "marc_Descripcion"];
  ...
  const { handleSubmit, register, reset, control, watch, formState, setValue } =
    useForm({
      defaultMarcasCarrosValues, //Campos del formulario
      mode: "all",
  ...
  const MarcaCarroGetData = async () => {
    try {
      setCargandoData([]);
      setData([]);

      const data = await marcasCarrosServices.listar();

      setData(data);
      data.length > 0 ? setCargandoData(data) : setCargandoData(null);
      SetExportData(await marcasCarrosServices.ExportData());
    } catch (error) {
  ...
  const MarcaCarroCreate = async () => {
    try {
      const response = await marcasCarrosServices.crear(datosWatch);
      //;
  ...
  const MarcaCarroEdit = async () => {
    try {
      const response = await marcasCarrosServices.editar(datosWatch);
      // ;
      if (response.data.data.messageStatus == "1") {
        ToastSuccessEditar();
        MarcaCarroGetData();
  ...
  const MarcaCarroDelete = async () => {
    try {
      const response = await marcasCarrosServices.eliminar(datosWatch);
      //;
      ...
      DialogEliminar();
      ToastError("Registro no Borrado.");
    }
  }
  ...
  return (
    <Card sx={{ minWidth: 275, margin: "40px" }}>
      <CardMedia
        component="img"
        height="200"
        image="https://i.ibb.co/fDL4fmc/MARCAS-DE-AUTOMOVIL.png"
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={editar ? "Editar Marca De Vehículo" : "Agregar Marca de Vehículo"}
                  />
                </Divider>
              </Grid>
              <Grid item xs={2}>
              ...
              <Grid item xs={8}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.marc_Descripcion}>Descripción:</FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
                        {...field}
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} style={{ marginBottom: "30px" }}>
              <Divider style={{ marginTop: "0px" }}>
                <Chip label="Detalles de la Marca" />
              </Divider>
            </Grid>
            ...
            >
              <Box sx={{ textAlign: "center" }}>
                <InputLabel htmlFor="id">
                  ...
                  <Typography>{DatosDetalles["marc_Id"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
            ...
            >
              <Box sx={{ textAlign: "center" }}>
                <InputLabel htmlFor="descripcion">
                  ...
                  <Typography>{DatosDetalles["marc_Descripcion"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
              ...
              >
                ...
                <tbody>
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["marc_FechaCreacion"]
                        ? new Date(
                          DatosDetalles["marc_FechaCreacion"]
                        ).toLocaleString()
                        : ""}
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["marc_FechaModificacion"]
                        ? new Date(
                          DatosDetalles["marc_FechaModificacion"]
                        ).toLocaleString()
                        : ""}
