...
function NavbarStyle2Content(props) {
  return (
    <Root className={clsx('flex flex-auto flex-col overflow-hidden h-full', props.className)}>
      ...
      <StyledContent  className="flex flex-1 flex-col min-h-0" option={{ suppressScrollX: true, wheelPropagation: false }}>
      <UserNavbarHeader />

        <Navigation layout="vertical" />

        <div className="flex flex-0 items-center justify-center py-48 opacity-10">
          {/* <img className="w-full max-w-96" src="https://i.ibb.co/s2Zmg1j/logo.png" alt="footer logo" /> */}
        </div>
      </StyledContent>
    </Root>
  );
