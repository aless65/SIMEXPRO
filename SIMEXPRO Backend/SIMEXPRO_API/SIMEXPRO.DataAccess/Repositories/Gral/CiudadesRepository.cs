...
namespace SIMEXPRO.DataAccess.Repositories.Gral
{
    public class CiudadesRepository : IRepository<tbCiudades>
    {
        ...
        public RequestStatus Insert(tbCiudades item)
        {
            ...
            var answer = db.QueryFirst<string>(ScriptsDataBase.CiudadesPorProvincia, parametros, commandType: CommandType.StoredProcedure);
            result.MessageStatus = answer;
            return result;
        }
        ...
        public IEnumerable<tbCiudades> CiudadesPorProvincia(tbCiudades item)
        {
            using var db = new SqlConnection(SIMEXPRO.ConnectionString);
            RequestStatus result = new RequestStatus();
            var parametros = new DynamicParameters();
